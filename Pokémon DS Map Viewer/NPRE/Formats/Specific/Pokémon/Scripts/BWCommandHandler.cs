...
namespace NPRE.Formats.Specific.Pokémon.Scripts
{
    public class BWCommandHandler
    {
        ...
        public Scripts.Commands_s readCommandBW(BinaryReader reader, Scripts.Commands_s com, List<uint> movOffsetList, List<uint> functionOffsetList, List<uint> scriptStartList, List<uint> scriptOffList, List<uint> scriptOrder)
        {
        ...
        public void getCommandSimplifiedBW(string[] scriptsLine, ref int lineCounter, string space, List<int> visitedLine)
        {
            ...
            switch (commandList[2])
            {
                ...
                case "SetStackDerefVar(09)":
                    ...
                    bool setVarsetVarConditionBlock = scriptsLine[tempLineCounter].Contains("SetStackDerefVar(09)") && scriptsLine[tempLineCounter + 1].Contains("SetStackDerefVar(09)") && scriptsLine[tempLineCounter + 2].Contains("Condition");
                    if (setVarCompareConditionBlock || setVarsetVarConditionBlock)
                    {
                        while ((scriptsLine[tempLineCounter].Contains("SetStackDerefVar(09)") || scriptsLine[tempLineCounter].Contains("StoreFlag")) && scriptsLine[tempLineCounter + 1].Contains("SetStackVar(08)")
                            || scriptsLine[tempLineCounter].Contains("SetStackDerefVar(09)") && scriptsLine[tempLineCounter + 1].Contains("SetStackDerefVar(09)") && scriptsLine[tempLineCounter + 2].Contains("Condition"))
                        {
                            numSetVar++;
        ...
        private void readFunctionSimplified(string[] scriptsLine, string space, ref int functionLineCounter, ref string line2, List<int> visitedLine)
        {
            do
            {
                ...
                if (line2.Length > 1)
                {
                    ...
                    try
                    {
                        getCommandSimplifiedBW(scriptsLine, ref functionLineCounter, space + "   ", visitedLine);
                    }
                    catch
                    {
                    }
        ...
        public void PrintCommand(RichTextBox scriptBox, Scripts.Commands_s command, List<uint> scriptOrder)
        {
            ...
            switch (command.Name.Split('(')[0])
            {
                ...
                case "Goto":
                    ...
                    break;
                case "GenericMessage":
                    if (command.parameters[1] < 0x4000)
                        idMessage = (int)command.parameters[1];
                    if (textFile != null && idMessage < textFile.messageList.Count)
                        scriptBox.AppendText(StoreHex((int)command.parameters[0]) + StoreHex((int)command.parameters[1]) + StoreHex((int)command.parameters[2]) + StoreHex((int)command.parameters[3]) + "= ' " + textFile.messageList[idMessage] + " '");
                    else
                        scriptBox.AppendText(StoreHex((int)command.parameters[0]) + StoreHex((int)command.parameters[1]) + StoreHex((int)command.parameters[2]) + StoreHex((int)command.parameters[3]));
                    break;
                default:
